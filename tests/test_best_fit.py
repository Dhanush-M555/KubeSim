#!/usr/bin/env python
"""
Real application test for the Best-Fit scheduling algorithm.
This test verifies that the Best-Fit algorithm correctly places pods on the node that will have the least remaining capacity.
"""
...
def test_best_fit_algorithm():
    """
    Test that the Best-Fit scheduling algorithm works correctly.
    
    Best-Fit places pods on the node that will have the least remaining capacity after placement.
    
    This test:
    1. Updates config to use best-fit algorithm
    2. Starts the application
    3. Creates nodes with different capacities
    4. Launches pods and verifies they go to the "tightest fit" node
    5. Verifies the placement is correct according to best-fit behavior
    """
    ...
    try:
        ...
        # Step 5: Test the best-fit algorithm with a 3-core pod
        # Current state:
        # - Node 1: 1 core available (not enough for a 3-core pod)
        # - Node 2: 2 cores available (not enough for a 3-core pod)
        # - Node 3: 8 cores available
        # Best-fit should choose Node 3 (it's the only one with enough capacity)
        
        print("\nTesting best-fit with a 3-core pod...")
        test_pod1_response = requests.post(
            f"{api_url}/launch-pod",
            json={"pod_id": "test_pod_1", "cpu": 3}
        )
        ...
        print(f"Test Pod 1 (3 cores) placed on node: {test_pod1_node}")
        
        # Best-fit should have placed it on node 3 (only node with enough capacity)
        if test_pod1_node != node3_id:
            print(f"ERROR: Best-fit algorithm failed. Test Pod 1 should be on node {node3_id} but is on {test_pod1_node}")
            return False
        
        # Step 6: Test with a 2-core pod
        # Current state after pod 1 placement:
        # Let's recheck node capacities
        time.sleep(2)
        updated_status_response = requests.get(f"{api_url}/pod-status")
        updated_pod_status = updated_status_response.json()
        ...
        # Current state:
        # - Node 1: 1 core available 
        # - Node 2: 2 cores available 
        # - Node 3: 5 cores available (after placing test_pod_1)
        # Best-fit should choose Node 2 (leaves least remaining capacity)
        
        print("\nTesting best-fit with a 2-core pod...")
        ...
        print(f"Test Pod 2 (2 cores) placed on node: {test_pod2_node}")
        
        # Best-fit should have placed it on node 2
        if test_pod2_node != node2_id:
            print(f"ERROR: Best-fit algorithm failed. Test Pod 2 should be on node {node2_id} but is on {test_pod2_node}")
            return False
        
        # Step 7: Test with a 1-core pod
        # Check current state after pod 2 placement
        time.sleep(2)
        updated_status_response = requests.get(f"{api_url}/pod-status")
        updated_pod_status = updated_status_response.json()
        updated_nodes_response = requests.get(f"{api_url}/list-nodes")
        ...
        print("\nTesting best-fit with a 1-core pod...")
        ...
        # Determine which node should have been chosen based on available capacity
        expected_node = None
        
        # Check if node_2 has enough capacity for a 1-core pod
        if node_capacities.get(node2_id, 0) >= 1:
            # If node_2 has enough capacity, it should be chosen (best fit)
            expected_node = node2_id
        else:
            # Otherwise, find the node with the least remaining capacity that can fit the pod
            viable_nodes = {node_id: capacity for node_id, capacity in node_capacities.items() if capacity >= 1}
            if viable_nodes:
                expected_node = min(viable_nodes.items(), key=lambda x: x[1])[0]
        
        # If no viable nodes or expected_node is not set, test would fail anyway, so use node_1 as fallback
        if not expected_node:
            expected_node = node1_id
            
        print(f"Expected node for pod 3: {expected_node} based on available capacity")
            
        # Best-fit should have placed it on the expected node
        if test_pod3_node != expected_node:
            print(f"ERROR: Best-fit algorithm failed. Test Pod 3 should be on node {expected_node} but is on {test_pod3_node}")
            return False
        ...
        print("\nSUCCESS: Best-fit scheduling algorithm works correctly!")
        return True
...
if __name__ == "__main__":
    # Run the test
    success = test_best_fit_algorithm()
